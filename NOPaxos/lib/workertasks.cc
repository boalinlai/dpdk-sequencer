...
taskid_t WorkerTasks::CreateTask() {
...
void WorkerTasks::CompleteTask(taskid_t taskid, task_t task) {
  std::unique_lock<std::mutex> lck(tasks_mutex_);
  task.done = true;
  tasks_[taskid] = task;
  task_cv_.notify_all();
...
task_t WorkerTasks::PullNextCompletedTask() {
  std::unique_lock<std::mutex> lck(tasks_mutex_);
...
bool WorkerTasks::IsEmpty() { return tasks_.empty(); }

void WorkerTasks::Stop() {
  done_ = true;
  task_cv_.notify_all();
}
